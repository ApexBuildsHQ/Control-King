...
// 1. خريطة توحيد مسميات الأزرار الشاملة
const GLOBAL_BUTTON_MAP = {
  // TV & Receivers
  'PWR': 'power', 'POWER': 'power', 'POWER_OFF': 'power', 'ON/OFF': 'power', 'STANDBY': 'power', 'OFF': 'power', 'ON': 'power',
  ...
  'COOL': 'mode_cool', 'HEAT': 'mode_heat', 'FAN_ONLY': 'mode_fan', 'DRY': 'mode_dry', 'AUTO_MODE': 'mode_auto', 'AUTO': 'mode_auto',
  ...
  // Smart Lighting
  'BRIGHT_UP': 'brightness_up', 'BRIGHTEN': 'brightness_up', 'BRIGHT_DOWN': 'brightness_down', 'DIM': 'brightness_down',
...
// 2. تصنيف الأجهزة الدقيق لمنع أي تسرب لأقسام مثل climate أو light
...
function normalizeButton(rawBtn, defaultProtocol = 'NEC', defaultFreq = 38000) {
  ...
  const codeVal = rawBtn.code || (rawBtn.ir && rawBtn.ir.code) || (rawBtn.wifi && rawBtn.wifi.endpoint);

  // استبعاد أي زر لا يملك شفرة حقيقية
  if (!codeVal || String(codeVal).trim() === '' || codeVal === 'undefined' || codeVal === 'null') {
    return null;
  }
...
// تفكيك الأوامر المتداخلة في ملفات SmartIR
function extractSmartIRCommands(obj, prefix = '') {
  let cmds = [];
  if (typeof obj === 'string' || typeof obj === 'number') {
    cmds.push({ key: prefix, code: String(obj) });
  } else if (typeof obj === 'object' && obj !== null) {
    Object.keys(obj).forEach(k => {
      const newKey = prefix ? `${prefix}_${k}` : k;
      cmds = cmds.concat(extractSmartIRCommands(obj[k], newKey));
    });
  }
  return cmds;
}
  ...
  files.forEach(filePath => {
    try {
      ...
      const buttons = (parsed.buttons || [])
        .map(b => normalizeButton(b, parsed.protocol, parsed.frequency))
        .filter(Boolean);
  ...
  categories.forEach(category => {
    ...
    files.forEach(filePath => {
      ...
      const buttons = rawButtons
        .map(b => normalizeButton(b, 'NEC', 38000))
        .filter(Boolean);
  ...
  categories.forEach(category => {
    ...
    files.forEach(filePath => {
      try {
        ...
        let rawButtons = [];
        if (data.commands) {
          rawButtons = extractSmartIRCommands(data.commands).map(item => ({
            key: item.key,
            code: item.code,
            type: 'IR'
          }));
        ...
        const buttons = rawButtons
          .map(b => normalizeButton(b, data.protocol || 'RAW', 38000))
          .filter(Boolean);
...
function writeOutputFiles() {
  // تنظيف مجلد المخرجات بالكامل قبل الكتابة لمنع وجود ملفات قديمة
...
function run() {
  console.log('🚀 Starting Clean Remote Data Processing...');
  parseLocalRawData();
  cloneRepositories();
  parseFlipperFiles();
  parseSmartIRFiles();
  ...
  if (fs.existsSync(TEMP_DIR)) fs.rmSync(TEMP_DIR, { recursive: true, force: true });
  console.log('🎉 Processing Complete! IR and Wi-Fi data perfectly consolidated.');
}
